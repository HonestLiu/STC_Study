...
void PCF8563_set_timer(Time_Freq freq, u8 countDown) {
    u8 p = 0;
    //1.定时器控制寄存器，使能定时器和选择定时器时钟源
    p = freq + 0x80;
    I2C_WriteNbyte(PCF8563_DEV_ADDR, 0x0E, &p, 1);
    //2.定时器寄存器，设置定时器的倒计时周期
    p = countDown;//倒计时值为countDown
    I2C_WriteNbyte(PCF8563_DEV_ADDR, 0x0F, &p, 1);
